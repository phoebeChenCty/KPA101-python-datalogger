...
sampling_interval = 2  # interval seconds, how fast the code samples the poll
...
try:
    ...
    print('Thor_logger: [antenna %s] [kpa101 polling %i ms] [file sampling %2.3f s]' % (
        ap,  polling_interval, sampling_interval))
    print('Kinesis serials numbers found: ', list_devices())

    kpa101 = KCubePositionAligner.CreateKCubePositionAligner(serial_id)
    print("define kpa101")

    try:
        kpa101.Connect(serial_id)
        print("connected!!!")
    except:
